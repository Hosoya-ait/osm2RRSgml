...
//        System.out.println("distance:"+distance);
    ...
    private void ConnectRoad(String node){
        int roadNum = 0;
        ArrayList<String> tmpRoadNodeList = new ArrayList<String>();
        ArrayList<String> tmpRemoveRoad = new ArrayList<String>();
        ...
        for (int i=1; i<=Integer.parseInt(rm.getRoadNodeID()); i++) {
            if (rm.getRoadNodeList(String.valueOf(i)).contains(node)) {
                ...
                tmpRemoveRoad.add(String.valueOf(i));
            }
        }
        ...
        switch (roadNum) {
            case 1:
                break;
            case 2:
                double degree00 = calcDegreePoint(node, tmpRoadNodeList.get(0));
                if (degree00 < 0) {
                    degree00+=360;
                }
                double degree01 = calcDegreePoint(node, tmpRoadNodeList.get(1));
                if (degree01 < 0) {
                    degree01+=360;
                }
                double degree02 = calcDegreePoint(node, tmpRoadNodeList.get(2));
                if (degree02 < 0) {
                    degree02+=360;
                }
                double degree03 = calcDegreePoint(node, tmpRoadNodeList.get(3));
                if (degree03 < 0) {
                    degree03+=360;
                }
                ...
                rm.setRemoveRoadList(tmpRemoveRoad.get(0));
                rm.setRemoveRoadList(tmpRemoveRoad.get(1));
                break;
            ...
            default:
                for (int i = 0; i <tmpRoadNodeList.size() ;i+=2 ) {
                    ...
                    for (int k = 1; k< tmpRoadNodeList.size();k+=2 ) {
                        if (k-1 != i) {
                            double degree1 = calcMiddleDegreePoint(node, tmpRoadNodeList.get(i),tmpRoadNodeList.get(i+1));

                            if (degree1 < 0) {
                                degree1+=360;
                            }
                            double degree2 = calcMiddleDegreePoint(node , tmpRoadNodeList.get(k-1), tmpRoadNodeList.get(k));
                    ...
                    if (minDegree<60) {
                        ...
                        array1.add(node);
                        array1.add(tmpRoadNodeList.get(usePoint));
                        array1.add(tmpRoadNodeList.get(i));
                        array1.add(node);
                        rm.setTmpRoadList(array1);
                    }
                }
